<?php session_start(); 
	...
	$nsx=isset($_REQUEST['camera'])?$_REQUEST['camera']:"";
 ?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
	...
	<div class="container text-center" style="margin-top: 30px;">
		<div class="row">
			<div class="col-12 text-center">
				<h1 class="gold-man display-4 wow animate__bounceInUp" style="text-transform: uppercase;font-size: 250%; text-shadow: 1px 1px 20px;">đối tác chiến lược</h1>
			</div>
		</div>
	...
	<?php 
		...
		$sql=($nsx=="") ? "select * from sanpham" : "select * from sanpham join nhasanxuat on nhasanxuat.manhasanxuat=sanpham.manhasanxuat where nhasanxuat.tennhasanxuat='".$nsx."'";
		// echo $sql;
		// if($nsx==""){
		...
		if(true){
			if($page < 1 || $page > $number_of_pages || !is_numeric($page)){
					...
					echo '<div class="row mb-2">';
						echo '<div class="col-12 align-content-start quicksand">';
							echo '<div class="container">';
								...
								$sortLen=($nsx=="")?"sort=ASC&page=1":"camera=".$nsx."&sort=ASC&page=1";
								echo '<a href="'.$_SERVER['PHP_SELF'].'?page=1" class="btn btn-dark">Tăng dần</a>';
								$sortXuong=($nsx=="")?"sort=DESC&page=1":"camera=".$nsx."&sort=DESC&page=1";
								echo '<a href="'.$_SERVER['PHP_SELF'].'?page=1" id="sort" class="btn btn-dark">Giảm dần</a>';
							echo '</div>';
						echo '</div>';
					echo "</div>";
				echo '</div>';
				echo '<div class="container-fluid">';
					...
					// $sql=($nsx=="") ? "select * from sanpham" : "select * from sanpham join nhasanxuat on nhasanxuat.manhasanxuat=sanpham.manhasanxuat where nhasanxuat.tennhasanxuat='".$nsx."'";
					$sql=($nsx=="")?"select * from sanpham order by sanpham.masanpham DESC LIMIT ".$page_first_result.",".$result_per_page:"select * from sanpham join nhasanxuat on nhasanxuat.manhasanxuat=sanpham.manhasanxuat where nhasanxuat.tennhasanxuat='".$nsx."' order by sanpham.masanpham DESC LIMIT ".$page_first_result.",".$result_per_page;
		...
		echo '</div>';
				...
				echo '<div class="row mb-2">';
					echo '<div class="col-12 align-content-start quicksand">';
						echo '<div class="container">';
							echo 'Sắp xếp theo giá:';
							$sortLen=($nsx=="")?"sort=ASC&page=1":"camera=".$nsx."&sort=ASC&page=1";
							echo '<a href="'.$_SERVER['PHP_SELF'].'?'.$sortLen.'" class="btn btn-dark">Tăng dần</a>';
							$sortXuong=($nsx=="")?"sort=DESC&page=1":"camera=".$nsx."&sort=DESC&page=1";
							echo '<a href="'.$_SERVER['PHP_SELF'].'?'.$sortXuong.'" id="sort" class="btn btn-dark">Giảm dần</a>';
						echo '</div>';
					echo '</div>';
				echo "</div>";
			echo '</div>';
			echo '<div class="container-fluid">';
				...
				$sql="select * from sanpham join nhasanxuat on nhasanxuat.manhasanxuat=sanpham.manhasanxuat where nhasanxuat.tennhasanxuat='".$nsx."' order by dongia ".$sort." LIMIT ".$page_first_result.",".$result_per_page;
				$result=mysqli_query($ketnoi,$sql);
			...
			if($page>1){
				...
				echo '<li class="page-item">';
					...
					echo '<a href="'.$_SERVER['PHP_SELF'].'?sort='.$sort.'&page=1" class="page-link">Đầu</a>';
				echo '</li>';

				//Hiển thị nút Previous
				echo '<li class="page-item">';
				echo '<a href="'.$_SERVER['PHP_SELF'].'?sort='.$sort.'&page='.$previous.'" class="page-link">&larr;</a>';
				echo '</li>';
				...
				for($i=$page-4;$i<$page;$i++){
					if($i>0){
						...
						echo '<a href="'.$_SERVER['PHP_SELF'].'?sort='.$sort.'&page='.$i.'" class="page-link">'.$i.'</a>';
						echo '</li>';
					}
				}
			}else{
			...
			//Hiển thị nút chuyển page hiện hành
			echo '<li class="page-item active">';
			echo '<a href="'.$_SERVER['PHP_SELF'].'?sort='.$sort.'&page='.$page.'" class="page-link">'.$page.'</a>';
			echo '</li>';
			...
			if($page!=$number_of_pages){
				...
				//Hiển thị nút Next
				echo '<li class="page-item">';
				echo '<a href="'.$_SERVER['PHP_SELF'].'?sort='.$sort.'&page='.$next.'" class="page-link">	&rarr;</a> ';
				echo '</li>';

				//Hiển thị nút chuyển đến page cuối
				echo '<li class="page-item">';
				echo '<a href="'.$_SERVER['PHP_SELF'].'?sort='.$sort.'&page='.$number_of_pages.'" class="page-link">Cuối</a>';
				echo '</li>';
			}else{
				//Hiển thị nút Next
				echo '<li class="page-item disabled">';
